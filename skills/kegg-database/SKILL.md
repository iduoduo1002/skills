...
### Workflow: Compound to Pathway Analysis
...
### Workflow: Cross-Database Integration

```python
from scripts.kegg_api import kegg_conv, kegg_get

# Convert KEGG gene IDs to external database IDs
...
2. **Genetic Information Processing** (e.g., `map03010` - Ribosome)
3. **Environmental Information Processing** (e.g., `map04010` - MAPK signaling)
4. **Cellular Processes** (e.g., `map04140` - Autophagy)
5. **Organismal Systems** (e.g., `map04910` - Insulin signaling)
6. **Human Diseases** (e.g., `map05200` - Pathways in cancer)
7. **Drug Development**
...
1. Maximum 10 entries per operation (image/kgml/json: 1 entry only)
2. Academic use only; commercial use requires licensing
...
**Image/KGML errors**: These formats only work with single entries
